...
#[cfg(feature = "bare_value_in_gc_struct_lint")]
mod bare_value_in_gc_struct;
...
impl Callbacks for MyCallbacks {
    fn config(&mut self, config: &mut Config) {
        config.register_lints = Some(Box::new(move |sess, lint_store| {
            ...
            #[cfg(feature = "bare_value_in_gc_struct_lint")]
            bare_value_in_gc_struct::register(lint_store);
        }));
    }
}
...
fn main() {
    ...
    // Pass cfg(crown) and register the `crown` tool namespace so that
    // `#[crown::...]` attributes resolve in all crates without manual
    // `#![register_tool(crown)]` boilerplate.
    args.extend([
        "--cfg".to_owned(),
        "crown".to_owned(),
        "--check-cfg=cfg(crown)".to_owned(),
        "-Zcrate-attr=feature(register_tool)".to_owned(),
        "-Zcrate-attr=register_tool(crown)".to_owned(),
    ]);
