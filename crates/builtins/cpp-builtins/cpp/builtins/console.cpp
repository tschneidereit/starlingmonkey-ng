...
static bool time(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  if (!timer_map.contains(label)) {
    ...
    timer_map[label] = std::chrono::steady_clock::now();
  }
...
bool install(JSContext *cx, HandleObject global) {
  ...
  JS::RootedObject console(cx,
                           JS_NewObjectWithGivenProto(cx, &Console::class_, proto));
  if (!console) {
    return false;
  }
  ...
  if (!JS_DefineProperties(cx, console, Console::properties)) {
    return false;
  }
  return JS_DefineFunctions(cx, console, Console::methods);
}
} // namespace builtins::web::console

extern "C" {
bool console_install(JSContext *cx, JSObject *global_raw) {
  JS::RootedObject global(cx, global_raw);
  return builtins::web::console::install(cx, global);
}
}
